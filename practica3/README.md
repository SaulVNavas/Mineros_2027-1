...
**R.**
    Las cifras difieren porque la media simple asume erróneamente que cada persona en la base de datos tiene exactamente la misma probabilidad de haber sido encuestada, lo que hace que todos los registros tengan el mismo nivel de aportación al total; no obstante, el factor de expansión añade valores de importancia para abarcar el encapsulamiento de sectores poblacionales en representaciones que, sin dicho factor, se consideran para una única persona.
...
**R.**
    Si la situación es verdadera, entonces podemos plantear una hipótesis sobre la distribución, declarando que la concentración de situaciones de violencia de pareja no se da hacia un rango de edades concreto, sino que puede ser una mezcla entre las edades consideradas como tempranas, por causas derivadas de la alta vulnerabilidad de esos sectores; y las consideradas como tardías, por causas diferentes a las primeras mencionadas.
